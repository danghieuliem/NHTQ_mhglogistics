...
const { globalCRUD, put, post, get } = new BaseAPI<
  TUserCartOrderShopTemp,
  TFilterParams
>("order-shop-temp");
...
export const orderShopTemp = {
  ...
  updateNote: (params: { Note: string; Id: number }) =>
    put("/update-note", undefined, { params }),

  payment: (data: TUserPayment) => post("/payment", data),

  getTotalPrice: (params: { id: number[] }) =>
    get("/get-total-price", { params }),

  addSame: (params: { Id: number }) => post("/add-same", { ...params }),
  addToCart: (data: any) => post("", data),
};
