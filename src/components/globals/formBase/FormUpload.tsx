...
export const FormUpload = <TFieldValues extends FieldValues = FieldValues>({
  ...
  const handleChange = async (info: UploadChangeParam<UploadFile<any>>) => {
    ...
    try {
      ...
      if (image) {
        _format.getBase64(info.file?.originFileObj, (image) =>
          setFileList(image),
        )
      } else {
        setFileList([...info.fileList])
    ...
    <React.Fragment>
      ...
      <div>
        <Upload
          ...
          fileList={
            !image && typeof fileList !== 'string' ? fileList : undefined
          }
        >
          {loading ? (
            'Đang tải...'
            ...
            children ?? (
              <i className={clsx('fas fa-plus-circle', 'text-xl')}></i>
            )
          )}
        </Upload>
        <ErrorMessage
          ...
          name={name as any}
          render={({ message }) => (
            <p className='mt-1 text-xs font-medium text-warning'>{message}</p>
          )}
        />
      </div>
    </React.Fragment>
