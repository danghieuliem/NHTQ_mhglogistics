				...
				<div className="grid grid-cols-6 gap-3">
					...
					<div className="col-span-3">
						<FormSelect
							...
							rules={{
								validate: (val) => typeof val === "boolean" || "This is required field",
							}}
						/>
					</div>
					<div className="col-span-2">
						<FormInputNumber
							...
							rules={{
								...
								validate: (val: number) => _format.isNumber(val.toString()) || "Không đúng định dạng số",
							}}
						/>
					</div>
					<div className="col-span-2">
...
const Update = ({wareHouseFrom, wareHouse, shippingType, idTarget, data, onCancel}) => {
	...
	const _onPress = (data: TTariffTQVN) => {
		const newData = {
			...
			ShippingTypeToWareHouseName: changeName.current.nameShippingTypeToWareHouseName,
		};
				...
				<div className="grid grid-cols-6 gap-3">
					<div className="col-span-3">
						<FormSelect
							...
							callback={(val) => {
								changeName.current.nameWareHouseFrom = wareHouseFrom.find((item) => item.Id === val)?.Name;
							}}
							rules={{required: "Vui lòng chọn kho TQ!"}}
						/>
					</div>
					<div className="col-span-3">
						<FormSelect
							...
							callback={(val) => {
								changeName.current.nameWareHouseTo = wareHouse.find((item) => item.Id === val)?.Name;
							}}
							select={{label: "Name", value: "Id"}}
							rules={{required: "Vui lòng chọn kho VN!"}}
						/>
					...
					<div className="col-span-3">
						<FormSelect
							...
							callback={(val) => {
								changeName.current.nameShippingTypeToWareHouseName = shippingType.find((item) => item.Id === val)?.Name;
							}}
							placeholder="Chọn hình thức vận chuyển"
						/>
					</div>
					<div className="col-span-3">
						<FormSelect
							...
							callback={(val) => {
								changeName.current.nameHelpMovingName = categoryData.find((item) => item.Id === !!val)?.Name;
							}}
							placeholder="Chọn loại đơn hàng"
							rules={{
								validate: (val) => typeof val === "boolean" || "This is required field",
...
export const TariffChinaVietNamForm: FC<
	TForm<TTariffTQVN> & {refetch: () => void; type: string; idTarget?: number; defaultValues?: any}
	...
	return (
		<Modal visible={visible} onCancel={onCancel}>
			<FormCard>
				...
				{type === "addNew" ? (
					<AddNew wareHouse={wareHouse} wareHouseFrom={wareHouseFrom} shippingType={shippingType} onCancel={onCancel} />
				) : (
					<Update
						onCancel={onCancel}
						wareHouse={wareHouse}
