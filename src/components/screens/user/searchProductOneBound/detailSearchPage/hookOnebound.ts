...
export const useHookOnebound = () => {
  ...
  const [data, setData] = useState<TItemDetail>()
  ...
  const { mutateAsync } = useMutation((router: NextRouter) => {
    ...
    if (!router.isReady) return Promise.resolve(null)

    if (isUndefined(id) || isUndefined(curEcsite)) return Promise.resolve(null)
