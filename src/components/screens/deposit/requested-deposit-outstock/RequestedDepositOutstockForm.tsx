...
import { FormInput, FormSelect, toast } from '~/components'
...
export const RequestedDepositOutstockForm: React.FC<TProps> = ({ type }) => {
    ...
    if (
      ...
      Object.keys(watchArray()).find(
        (key) =>
          !!watchArray(key).find(
            (x) =>
              x.Status != ESmallPackageStatusData.ArrivedToVietNamWarehouse,
          ),
      )
    ) {
      alert(
  ...
  return (
    <React.Fragment>
      <div className='grid max-w-[500px] grid-cols-1 gap-4 px-4'>
